...
function emptyDB(){ return {users:[],posts:[],comments:[],likes:[],follows:[],reports:[],dictionary:[],notifications:[],messages:[],groups:[],groupMembers:[],groupMessages:[],channels:[],channelMembers:[],channelMessages:[],communities:[],communityMembers:[],verificationRequests:[],saves:[],messageAllows:[],stories:[],storyViews:[]}; }
function loadDB(){
  try {
...
app.get('/api/groups/:id/messages',auth,(req,res)=>{
  const g=db.groups.find(x=>x.id===req.params.id); if(!g)return res.status(404).json({error:'Группа не найдена'});
  if(!db.groupMembers.some(x=>x.groupId===g.id&&x.userId===req.user.id))return res.status(403).json({error:'Сначала вступите в группу'});
  res.json(db.groupMessages.filter(m=>m.groupId===g.id).sort((a,b)=>a.createdAt-b.createdAt).slice(-300));
});
app.post('/api/groups/:id/messages',auth,(req,res)=>{
  const g=db.groups.find(x=>x.id===req.params.id); if(!g)return res.status(404).json({error:'Группа не найдена'});
  if(!db.groupMembers.some(x=>x.groupId===g.id&&x.userId===req.user.id))return res.status(403).json({error:'Сначала вступите в группу'});
  const text=String(req.body.text||'').trim().slice(0,4000); const sticker=req.body.sticker?String(req.body.sticker).slice(0,500000):null;
  if(!text&&!sticker)return res.status(400).json({error:'Пустое сообщение'});
  const replyToId=String(req.body.replyToId||''); const r=replyToId?db.groupMessages.find(x=>x.id===replyToId&&x.groupId===g.id):null;
  const m={id:id(),groupId:g.id,from:req.user.id,userName:req.user.name,text,sticker,replyTo:r?{id:r.id,from:r.from,text:r.text||'',sticker:r.sticker||null}:null,createdAt:Date.now()};
  db.groupMessages.push(m); saveDB();
  db.groupMembers.filter(x=>x.userId!==req.user.id).forEach(x=>notify(x.userId,'group_message',{groupId:g.id,groupName:g.name,userId:req.user.id,userName:req.user.name,preview:text||sticker||'стикер'}));
  io.to(`group:${g.id}`).emit('group_message',m); res.json(m);
});
...
app.get('/api/channels/:id/messages',auth,(req,res)=>{
  const c=db.channels.find(x=>x.id===req.params.id); if(!c)return res.status(404).json({error:'Канал не найден'});
  const member=db.channelMembers.some(x=>x.channelId===c.id&&x.userId===req.user.id); if(!member)return res.status(403).json({error:'Подпишитесь на канал'});
  res.json(db.channelMessages.filter(m=>m.channelId===c.id).sort((a,b)=>a.createdAt-b.createdAt).slice(-300));
});
app.post('/api/channels/:id/messages',auth,(req,res)=>{
  const c=db.channels.find(x=>x.id===req.params.id); if(!c)return res.status(404).json({error:'Канал не найден'});
  if(c.ownerId!==req.user.id)return res.status(403).json({error:'Публиковать может только владелец канала'});
  const text=String(req.body.text||'').trim().slice(0,5000); if(!text)return res.status(400).json({error:'Пустая публикация'});
  const m={id:id(),channelId:c.id,from:req.user.id,userName:req.user.name,text,createdAt:Date.now()}; db.channelMessages.push(m);saveDB();
  db.channelMembers.filter(x=>x.userId!==req.user.id).forEach(x=>notify(x.userId,'channel_post',{channelId:c.id,channelName:c.name,userId:req.user.id,userName:req.user.name,preview:text}));
  io.to(`channel:${c.id}`).emit('channel_post',m);res.json(m);
});
