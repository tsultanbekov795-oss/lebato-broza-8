...
function emptyDB(){ return {users:[],posts:[],comments:[],likes:[],follows:[],reports:[],dictionary:[],notifications:[],testResults:[],streaks:[],messages:[],groups:[],groupMembers:[],groupMessages:[],channels:[],channelMembers:[],channelMessages:[],communities:[],communityMembers:[],verificationRequests:[],saves:[],messageAllows:[],stories:[],storyViews:[]}; }
function loadDB(){
  try {
    ...
    if(!Array.isArray(d.communities)) d.communities=[]; if(!Array.isArray(d.communityMembers)) d.communityMembers=[]; if(!Array.isArray(d.verificationRequests)) d.verificationRequests=[]; if(!Array.isArray(d.groups)) d.groups=[]; if(!Array.isArray(d.groupMembers)) d.groupMembers=[]; if(!Array.isArray(d.groupMessages)) d.groupMessages=[]; if(!Array.isArray(d.channels)) d.channels=[]; if(!Array.isArray(d.channelMembers)) d.channelMembers=[]; if(!Array.isArray(d.channelMessages)) d.channelMessages=[];
    for(const u of d.users){
      ...
      if(!Array.isArray(u.stickers)) u.stickers=[];
       if(!Array.isArray(u.stickerPacks)) u.stickerPacks=[];
...
app.post('/api/auth/register',async(req,res)=>{
  ...
  const user={id:id(),name,username,bio:'',avatar:defaultAvatar(name),passwordHash:await bcrypt.hash(password,10),createdAt:Date.now(),role:wantAdmin?'admin':'user',banned:false,blocks:[],stickers:[],stickerPacks:[],messagePrivacy:'everyone',verification:null};
...
app.get('/api/me/sticker-packs',auth,(req,res)=>res.json(req.user.stickerPacks||[]));
app.post('/api/me/sticker-packs',auth,(req,res)=>{const name=String(req.body.name||'').trim().slice(0,40);const stickers=Array.isArray(req.body.stickers)?req.body.stickers.filter(x=>typeof x==='string').slice(0,50):[];if(name.length<2)return res.status(400).json({error:'Название стикерпака слишком короткое'});if(!stickers.length)return res.status(400).json({error:'Добавьте хотя бы один стикер'});req.user.stickerPacks=req.user.stickerPacks||[];if(req.user.stickerPacks.some(p=>p.name.toLowerCase()===name.toLowerCase()))return res.status(409).json({error:'Такой стикерпак уже есть'});const pack={id:id(),name,stickers,createdAt:Date.now()};req.user.stickerPacks.unshift(pack);req.user.stickerPacks=req.user.stickerPacks.slice(0,20);saveDB();res.json(pack);});
app.delete('/api/me/sticker-packs/:id',auth,(req,res)=>{req.user.stickerPacks=(req.user.stickerPacks||[]).filter(p=>p.id!==req.params.id);saveDB();res.json({ok:true});});
...
app.post('/api/chats/:uid/messages',auth,(req,res)=>{
  ...
  const replySource=replyToId?db.messages.find(x=>x.id===replyToId&&((x.from===req.user.id&&x.to===to.id)||(x.from===to.id&&x.to===req.user.id))):null;
  const replyTo=replySource?{id:replySource.id,from:replySource.from,text:replySource.text||'',sticker:replySource.sticker||null}:null;
  const m={id:id(),from:req.user.id,to:to.id,text:text||'',sticker,replyTo,createdAt:Date.now()};
...
function groupView(g,uid){
  const members=db.groupMembers.filter(x=>x.groupId===g.id);
  return {...g,memberCount:members.length,isMember:members.some(x=>x.userId===uid),isOwner:g.ownerId===uid};
}
function channelView(c,uid){
  const members=db.channelMembers.filter(x=>x.channelId===c.id);
  return {...c,subscriberCount:members.length,isSubscribed:members.some(x=>x.userId===uid),isOwner:c.ownerId===uid};
}
app.get('/api/groups',auth,(req,res)=>{
  const mine=db.groupMembers.filter(x=>x.userId===req.user.id).map(x=>x.groupId);
  res.json(db.groups.filter(g=>mine.includes(g.id)||g.ownerId===req.user.id).map(g=>groupView(g,req.user.id)));
});
app.post('/api/groups',auth,(req,res)=>{
  const name=String(req.body.name||'').trim().slice(0,80);
  const description=String(req.body.description||'').trim().slice(0,300);
  if(name.length<2)return res.status(400).json({error:'Название слишком короткое'});
  const g={id:id(),name,description,ownerId:req.user.id,avatar:String(req.body.avatar||'').slice(0,500000),createdAt:Date.now()};
  db.groups.push(g); db.groupMembers.push({groupId:g.id,userId:req.user.id,role:'owner',createdAt:Date.now()}); saveDB();
  res.json(groupView(g,req.user.id));
});
app.post('/api/groups/:id/join',auth,(req,res)=>{
  const g=db.groups.find(x=>x.id===req.params.id); if(!g)return res.status(404).json({error:'Группа не найдена'});
  if(!db.groupMembers.some(x=>x.groupId===g.id&&x.userId===req.user.id)) db.groupMembers.push({groupId:g.id,userId:req.user.id,role:'member',createdAt:Date.now()});
  saveDB(); res.json(groupView(g,req.user.id));
});
app.delete('/api/groups/:id/leave',auth,(req,res)=>{
  const g=db.groups.find(x=>x.id===req.params.id); if(!g)return res.status(404).json({error:'Группа не найдена'});
  if(g.ownerId===req.user.id)return res.status(400).json({error:'Создатель не может выйти из своей группы'});
  db.groupMembers=db.groupMembers.filter(x=>!(x.groupId===g.id&&x.userId===req.user.id)); saveDB(); res.json({ok:true});
});
...
app.get('/api/groups/:id/messages',auth,(req,res)=>{
  ...
  if(!db.groupMembers.some(x=>x.groupId===g.id&&x.userId===req.user.id))
    return res.status(403).json({error:'Сначала вступите в группу'});
  const list=(db.groupMessages||[]).filter(m=>m.groupId===g.id)
...
app.post('/api/groups/:id/messages',auth,(req,res)=>{
  ...
  const text=String(req.body.text||'').trim().slice(0,4000);
  const sticker=req.body.sticker?String(req.body.sticker).slice(0,1000000):null;
  const media=req.body.media?String(req.body.media):null;
  const mediaType=req.body.mediaType==='video'?'video':(req.body.mediaType==='image'?'image':null);

  if(!text&&!sticker&&!media)
    return res.status(400).json({error:'Пустое сообщение'});

  if(media){
    if(!mediaType)return res.status(400).json({error:'Не указан тип медиа'});
    const max=mediaType==='video'?28000000:10000000;
    if(media.length>max)return res.status(413).json({error:'Файл слишком большой'});
    const prefix=mediaType==='video'?'data:video/':'data:image/';
    if(!media.startsWith(prefix)&&!media.startsWith('https://'))
      return res.status(400).json({error:'Некорректный медиафайл'});
  }

  const replyToId=String(req.body.replyToId||'');
  const r=replyToId?(db.groupMessages||[]).find(x=>x.id===replyToId&&x.groupId===g.id):null;
  const m={
    id:id(),groupId:g.id,from:req.user.id,userName:req.user.name,
    text,sticker,media,mediaType,
    replyTo:r?{id:r.id,from:r.from,text:r.text||'',sticker:r.sticker||null}:null,
    createdAt:Date.now()
  };

  db.groupMessages=db.groupMessages||[];
  db.groupMessages.push(m);
  saveDB();

  db.groupMembers.filter(x=>x.userId!==req.user.id).forEach(x=>{
    notify(x.userId,'group_message',{
      groupId:g.id,groupName:g.name,userId:req.user.id,userName:req.user.name,
      preview:text||sticker||(mediaType==='video'?'🎬 Видео':'📷 Фото')
    });
  });

  io.to(`group:${g.id}`).emit('group_message',m);
  res.json(m);
});

app.get('/api/channels',auth,(req,res)=>res.json(db.channels.map(c=>channelView(c,req.user.id))));
app.post('/api/channels',auth,(req,res)=>{
  const name=String(req.body.name||'').trim().slice(0,80); const description=String(req.body.description||'').trim().slice(0,500);
  if(name.length<2)return res.status(400).json({error:'Название слишком короткое'});
  const c={id:id(),name,description,ownerId:req.user.id,avatar:String(req.body.avatar||'').slice(0,500000),createdAt:Date.now()};
  db.channels.push(c); db.channelMembers.push({channelId:c.id,userId:req.user.id,role:'owner',createdAt:Date.now()}); saveDB(); res.json(channelView(c,req.user.id));
});
app.post('/api/channels/:id/subscribe',auth,(req,res)=>{
  const c=db.channels.find(x=>x.id===req.params.id); if(!c)return res.status(404).json({error:'Канал не найден'});
  if(!db.channelMembers.some(x=>x.channelId===c.id&&x.userId===req.user.id))db.channelMembers.push({channelId:c.id,userId:req.user.id,role:'subscriber',createdAt:Date.now()});
  saveDB();res.json(channelView(c,req.user.id));
});
app.delete('/api/channels/:id/subscribe',auth,(req,res)=>{
  db.channelMembers=db.channelMembers.filter(x=>!(x.channelId===req.params.id&&x.userId===req.user.id));saveDB();res.json({ok:true});
});
...
app.get('/api/channels/:id/messages',auth,(req,res)=>{
  ...
  const member=db.channelMembers.some(x=>x.channelId===c.id&&x.userId===req.user.id);
  if(!member)return res.status(403).json({error:'Подпишитесь на канал'});
  res.json((db.channelMessages||[]).filter(m=>m.channelId===c.id)
    .sort((a,b)=>a.createdAt-b.createdAt).slice(-300));
});
...
app.post('/api/channels/:id/messages',auth,(req,res)=>{
  ...
  if(c.ownerId!==req.user.id)
    return res.status(403).json({error:'Публиковать может только владелец канала'});
  ...
  if(!text&&!sticker&&!media)return res.status(400).json({error:'Пустая публикация'});
  ...
  const m={
    id:id(),channelId:c.id,from:req.user.id,userName:req.user.name,
    text,sticker,media,mediaType,createdAt:Date.now()
  };
  db.channelMessages=db.channelMessages||[];
  db.channelMessages.push(m);
  saveDB();

  db.channelMembers.filter(x=>x.userId!==req.user.id).forEach(x=>{
    notify(x.userId,'channel_post',{
      channelId:c.id,channelName:c.name,userId:req.user.id,userName:req.user.name,
      preview:text||sticker||(mediaType==='video'?'🎬 Видео':'📷 Фото')
    });
  });

  io.to(`channel:${c.id}`).emit('channel_post',m);
  res.json(m);
});


// Lebato test results -> notification.
app.post('/api/test-results',auth,(req,res)=>{
  const total=90;
  const score=Math.max(0,Math.min(total,Math.trunc(Number(req.body.score||0))));
  const percent=Math.round((score/total)*100);
  const title=String(req.body.title||'Тест завершён').slice(0,120);
  db.testResults=db.testResults||[];
  const createdAt=Date.now();
  const result={id:id(),userId:req.user.id,score,total,percent,title,createdAt};
  db.testResults.push(result);
  // Create the notification in the same transaction before responding.
  notify(req.user.id,'test_result',{resultId:result.id,score,total,percent,title,preview:`🏆 ${score}/${total} — ${percent}%`});
  saveDB();
  res.json({ok:true,score,total,percent,notificationCreated:true});
});
...
function streakKey(a,b){a=String(a);b=String(b);return a<b?`${a}:${b}`:`${b}:${a}`;}
function touchStreak(a,b){
  if(!a||!b||a===b)return null;
  const key=streakKey(a,b), today=new Date().toISOString().slice(0,10);
  let s=db.streaks.find(x=>x.key===key);
  if(!s){s={key,a:String(a),b:String(b),days:1,lastDate:today,active:true};db.streaks.push(s);return s;}
  if(s.lastDate===today)return s;
  const prev=new Date(s.lastDate+"T00:00:00Z"), cur=new Date(today+"T00:00:00Z");
  const diff=Math.round((cur-prev)/86400000);
  s.days=diff===1?s.days+1:1;s.lastDate=today;s.active=true;return s;
}
app.get('/api/streak/:userId',auth,(req,res)=>{
  const s=db.streaks.find(x=>x.key===streakKey(req.user.id,req.params.userId));
  res.json({streak:s||{days:0,active:false}});
});
...
// Scheduled availability for the Lebato test.
// Update is released on 2026-08-23; test opens on 2026-08-24.
app.get('/api/test-availability',(req,res)=>{
  const now=Date.now();
  const release=new Date('2026-08-23T00:00:00+02:00').getTime();
  const open=new Date('2026-08-24T00:00:00+02:00').getTime();
  res.json({
    released: now>=release,
    open: now>=open,
    releaseAt:'2026-08-23T00:00:00+02:00',
    openAt:'2026-08-24T00:00:00+02:00'
  });
});


// Secure password change for the current account.
app.post('/api/auth/change-password', auth, async (req,res)=>{
  try{
    const current=String(req.body?.currentPassword||'');
    const next=String(req.body?.newPassword||'');
    if(next.length<6) return res.status(400).json({error:'Новый пароль должен содержать минимум 6 символов'});
