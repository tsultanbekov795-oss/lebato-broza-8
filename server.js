...
function postView(p,viewerId){
  ...
  return {...p,author:u?.name||p.author||'Пользователь',username:u?.username||'',verification:u?.verification||null,avatar:u?.avatar||p.avatar||defaultAvatar(u?.name||'user'),likes,liked:db.likes.some(x=>x.postId===p.id&&x.userId===viewerId),saved:db.saves.some(x=>x.postId===p.id&&x.userId===viewerId),reposts:p.reposts||0,blocked};
}
...
app.get('/api/health',(req,res)=>res.json({ok:true,service:'Lebato Broza Social',version:'12'}));
...
app.post('/api/communities/:id/posts',auth,(req,res)=>{
  ...
  const p={id:id(),authorId:req.user.id,author:req.user.name,avatar:req.user.avatar,text,sticker,images,videos:[],hashtags:hashtags(text),reposts:0,comments:[],communityId:c.id,createdAt:Date.now(),editedAt:null};
...
app.post('/api/chats/:uid/messages',auth,(req,res)=>{
  const to=db.users.find(u=>u.id===req.params.uid),text=String(req.body.text||'').trim().slice(0,4000);
  ...
  const m={id:id(),from:req.user.id,to:to.id,text,createdAt:Date.now()};
  db.messages.push(m);saveDB();io.to(`user:${to.id}`).emit('message',m);notify(to.id,'message',{userId:req.user.id,userName:req.user.name});res.json(m);
});
