...
function emptyDB(){ return {users:[],posts:[],comments:[],likes:[],follows:[],reports:[],dictionary:[],notifications:[],messages:[]}; }
function loadDB(){
  try {
...
function publicUser(u,viewerId){
  ...
  return {id:u.id,name:u.name,bio:u.bio||'',avatar:u.avatar||defaultAvatar(u.name),createdAt:u.createdAt,role:u.role||'user',banned:!!u.banned,followers,following,isFollowing:!!(viewerId&&db.follows.some(x=>x.followerId===viewerId&&x.followingId===u.id)),isBlocked:!!(viewerId&&u.blocks?.includes(viewerId))};
}
...
function postView(p,viewerId){
  ...
  return {...p,author:u?.name||p.author||'Пользователь',avatar:u?.avatar||p.avatar||defaultAvatar(u?.name||'user'),likes,liked:db.likes.some(x=>x.postId===p.id&&x.userId===viewerId),reposts:p.reposts||0,blocked};
}
...
app.get('/api/health',(req,res)=>res.json({ok:true,service:'Lebato Broza Social',version:'6'}));
...
app.post('/api/auth/register',async(req,res)=>{
  ...
  const user={id:id(),name,bio:'',avatar:defaultAvatar(name),passwordHash:await bcrypt.hash(password,10),createdAt:Date.now(),role:(ADMIN_NAME===name.toLowerCase()?'admin':'user'),banned:false,blocks:[]};
...
app.get('/api/users',auth,(req,res)=>{const q=String(req.query.q||'').trim().toLowerCase();res.json(db.users.filter(u=>u.id!==req.user.id&&!u.banned&&(!q||u.name.toLowerCase().includes(q))).slice(0,50).map(u=>publicUser(u,req.user.id)));});
...
app.post('/api/posts',auth,(req,res)=>{
  const text=String(req.body.text||'').trim().slice(0,5000), sticker=req.body.sticker?String(req.body.sticker).slice(0,20):null;
  const images=Array.isArray(req.body.images)?req.body.images.slice(0,6).map(x=>String(x).slice(0,2000000)):[];
  if(!text&&!sticker&&!images.length)return res.status(400).json({error:'Пустой пост'});
  if(images.some(x=>x.length>2000000||(!x.startsWith('data:image/')&&!x.startsWith('https://'))))return res.status(400).json({error:'Некорректная картинка'});
  const p={id:id(),authorId:req.user.id,author:req.user.name,avatar:req.user.avatar,text,sticker,images,hashtags:hashtags(text),reposts:0,comments:[],createdAt:Date.now(),editedAt:null};
...
// Search.
app.get('/api/search',auth,(req,res)=>{const q=String(req.query.q||'').trim().toLowerCase().replace(/^#/,'');if(!q)return res.json({users:[],posts:[],hashtags:[]});const users=db.users.filter(u=>!u.banned&&u.name.toLowerCase().includes(q)).slice(0,20).map(u=>publicUser(u,req.user.id));const posts=db.posts.filter(p=>(p.text||'').toLowerCase().includes(q)||p.hashtags?.includes(q)).sort((a,b)=>b.createdAt-a.createdAt).slice(0,30).map(p=>postView(p,req.user.id));const hs=[...new Set(db.posts.flatMap(p=>p.hashtags||[]).filter(h=>h.includes(q)))].slice(0,20);res.json({users,posts,hashtags:hs});});
